#환경 설치 후에 다른 remote server를 통해서 진행해야할 듯!
...
# import torchvision.models.detection.utils
import natsort
import utils
# import os
...
dataset = PennFudanDataset('/mnt/ssm/algae_dataset/algae_total/train/resized_image',get_transform(train=False))
dataset_test = PennFudanDataset('/mnt/ssm/algae_dataset/algae_total/train/resized_image',get_transform(train=False))

indices = torch.randperm(len(dataset)).tolist()

# np.savez_compressed('/mnt/ssm/weighted_microcyistis/indices2', x=indices)
# np.savez_compressed('/mnt/ssm/weighted_membrane/indices', x=indices)
# load = np.load('/mnt/ssm/weighted_microcyistis/indices2.npz', allow_pickle=True)
...
#weighted maskr
#기존 maskr transfer learning을 위해 maskr weight 가져오기
model.load_state_dict(torch.load('/mnt/ssm/weighted_microcyistis/mask_model2.pth'))
...
# plt.imshow(pred.mul(255).permute(1,2,0).cpu())
# plt.imshow(lab['masks'].mul(255).permute(1,2,0).cpu())
# plt.show()
...
torch.save(model.state_dict(), '/mnt/ssm/weightedmask_elipse/4.1/Wmask_model.pth')
# torch.save(model.state_dict(), '/mnt/ssm/weightedmask_elipse/8/mask_model.pth')


#model load 후 visualize
# model.load_state_dict(torch.load('/mnt/ssm/weightedmask_elipse/8/mask_model.pth'))
# model.load_state_dict(torch.load('/mnt/ssm/mask_rcnn_pytorch.pth'))
# model.load_state_dict(torch.load('/mnt/ssm/weightedmask_elipse/4/mask_rcnn_pytorch.pth'))
model.load_state_dict(torch.load('/mnt/ssm/weightedmask_elipse/6.5/Wmask_model.pth'))
...
#
# def maskrcnn_loss(mask_logits, mask_proposals, gt_masks, gt_labels, pos_matched_idxs):
...
aa=torch.sum(W_masks[0],dim=0)
np.unique(aa)
plt.imshow(aa.mul(255))
plt.show()
...
W_targets = [
        project_masks_on_boxes(m, p, i, discretization_size)
        for m, p, i in zip(W_masks, mask_proposals, pos_matched_idxs)
    ]
...
mask_loss3 = torch.mean(W_targets * mask_loss)

aaaaaaa= W_targets * mask_loss
...
# train_one_epoch(model, optimizer, data_loader, device, 1, print_freq=10)
...
for images, targets in metric_logger.log_every(data_loader, print_freq=10, header=header):
